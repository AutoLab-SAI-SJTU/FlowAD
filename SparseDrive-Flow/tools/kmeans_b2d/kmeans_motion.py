...
def get_fut_agent(data_infos, idx, sample_rate, frames):
...
for idx in tqdm(range(len(data_infos))):
    ...
    boxes = info['gt_boxes']
    names = info['gt_names']
    names = [NameMapping[one] for one in names]
    # fut_masks = info['gt_agent_fut_masks']
    # trajs = info['gt_agent_fut_trajs']
    # velos = info['gt_velocity']

    trajs, fut_masks = get_fut_agent(data_infos, idx, 1, 6)
    ...
    for i in range(len(CLASSES)):
        ...
        mask = np.logical_and(
            fut_masks_cls.sum(axis=1) == motion_ts,
            distance < DIS_THRESH,
        )
...
clusters = []
for i in range(len(CLASSES)):
    intention_cls = np.concatenate(intention[i], axis=0).reshape(-1, motion_ts*2)
    if intention_cls.shape[0] < K:
        continue
    cluster = KMeans(n_clusters=K).fit(intention_cls).cluster_centers_
    cluster = cluster.reshape(-1, motion_ts, 2)
