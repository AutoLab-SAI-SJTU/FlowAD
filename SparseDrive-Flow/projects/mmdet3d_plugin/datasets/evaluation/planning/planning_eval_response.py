...
def planning_eval(results, eval_config, logger):
    ...
    for i, data in enumerate(tqdm(dataloader)):
        ...
        traj_tmp = res['img_bbox']['final_planning']
        x_dis = traj_tmp[-1][0] #  - traj_tmp[-2][0]
        relative_dis = abs(traj_tmp[-1][0] - sdc_planning[0][0][-1][0])
        if x_dis >= 2:
            ego_command = 0  # Turn Right
        elif x_dis <= -2:
            ego_command = 1  # Turn Left
        else:
            ego_command = 2  # Go Straight
        scene_id = data['img_metas'].data[0][0]['scene_id']
        if command != prev_command:
            added = False
        if scene_id == prev_scene_id:
            if not added:
                if ego_command == command and relative_dis < 0.5:
                    if not added:
                        cnt += 1
                        added = True
                else:
                    response_frame_total += 1
        else:
            added = False
    ...
    print('\n Avg Response Frame: ', response_frame_total/cnt, response_frame_total, cnt)
