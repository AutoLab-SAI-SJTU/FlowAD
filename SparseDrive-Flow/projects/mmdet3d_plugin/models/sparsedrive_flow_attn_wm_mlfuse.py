...
class SparseDrive_flow_attn_wm_mlfuse(BaseDetector):
    def __init__(
        self,
        img_backbone,
        head,
        ...
        super(SparseDrive_flow_attn_wm_mlfuse, self).__init__(init_cfg=init_cfg)
    ...
    def extract_feat(self, img, return_depth=False, metas=None):
        ...
        for idx, (mlvl_feat_flow_unfold, (H, W, flow_patch)) in enumerate(zip(mlvl_feats_flow_unfold, H_W_Ps)):
            if idx in self.flow_wm_ids:
                ...
                mlvl_feat_flow_unfold_fuse = torch.cat([rearrange(mlvl_feat_flow_unfold, 'b c h y p -> b h y p c'), rearrange(mlvl_feat_flow_unfold_fut.flip(dims=[1]), '(b n) t y c -> (b t n) y c', b=B, n=6, y=W//flow_patch).unsqueeze(2).unsqueeze(1).repeat(1,H,1,flow_patch,1)], dim=-1)
