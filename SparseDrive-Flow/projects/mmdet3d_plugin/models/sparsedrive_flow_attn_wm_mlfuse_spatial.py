...
class SparseDrive_flow_attn_wm_mlfuse_spatial(BaseDetector):
    def __init__(
        self,
        img_backbone,
        head,
        ...
        super(SparseDrive_flow_attn_wm_mlfuse_spatial, self).__init__(init_cfg=init_cfg)
        ...
        self.flow_wm_convs = nn.ModuleList([
            nn.Conv2d(embed_dims*3, embed_dims, kernel_size=3, padding=1) for flow_wm_id in self.flow_wm_ids
        ])
    ...
    def extract_feat(self, img, return_depth=False, metas=None):
        ...
        patch_quer_spatial = patch_query.clone()
        ...
        for idx, (mlvl_feat_flow_unfold, (H, W, flow_patch)) in enumerate(zip(mlvl_feats_flow_unfold, H_W_Ps)):
            if idx in self.flow_wm_ids:
                ...
                output_wm, mlvl_feat_flow_unfold_fut = self.world_models[layer_idx](rearrange(mlvl_feat_flow_unfold.mean(dim=2).mean(dim=-1), '(b t n) c y -> (b n) t y c', b=B, t=T, n=6).flip(dims=[1]), rearrange(patch_query, '(b t n) y c -> (b n) t y c', b=B, t=T, n=6).flip(dims=[1]))
                outputs_wm.append(output_wm)
                output_wm_spatial, mlvl_feat_flow_unfold_fut_spatial = self.world_models_spatial[layer_idx](rearrange(mlvl_feat_flow_unfold.mean(dim=2).mean(dim=-1), '(b t n) c y -> b y (t n) c', b=B, t=T, n=6), rearrange(patch_query, '(b t n) y c ->  b y (t n) c', b=B, t=T, n=6))
                outputs_wm_spatial.append(output_wm_spatial)

                mlvl_feat_flow_unfold_fuse = torch.cat([rearrange(mlvl_feat_flow_unfold, 'b c h y p -> b h y p c'), rearrange(mlvl_feat_flow_unfold_fut.flip(dims=[1]), '(b n) t y c -> (b t n) y c', b=B, n=6, y=W//flow_patch).unsqueeze(2).unsqueeze(1).repeat(1,H,1,flow_patch,1), rearrange(mlvl_feat_flow_unfold_fut_spatial, 'b y (t n) c -> (b t n) y c', b=B, n=6, y=W//flow_patch).unsqueeze(2).unsqueeze(1).repeat(1,H,1,flow_patch,1)], dim=-1)
