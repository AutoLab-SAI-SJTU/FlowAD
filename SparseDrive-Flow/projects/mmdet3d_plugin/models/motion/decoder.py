...
class HierarchicalPlanningDecoder(object):
    ...
    def decode(
        ...
        classification = classification.reshape(bs, self.cmd_type, self.ego_fut_mode)
        prediction = prediction.reshape(bs, self.cmd_type, self.ego_fut_mode, self.ego_fut_ts, 2).cumsum(dim=-2)
