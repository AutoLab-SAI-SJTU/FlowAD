...
load_from = None
resume_from = None
# resume_from = '/data3/guomingzhe/SparseDrive/work_dirs/sparsedrive_small_stage1_flow_attn_wm_mlfuse_ego_pe_spatial/iter_15060.pth'  # None
workflow = [("train", 1)]
fp16 = dict(loss_scale=32.0)
input_shape = (int(704*ratio), int(256*ratio))

flow_num = int(22 * ratio)
spat_size = int(6 * ratio)
...
model = dict(
    type="SparseDrive_flow_attn_wm_mlfuse_ego_pe_spatial",
    use_grid_mask=True,
    use_deformable_func=use_deformable_func,
    embed_dims=embed_dims,
    flow_patches=flow_patches,
    flow_ids=flow_ids,
    flow_attn_ids=flow_attn_ids,
    flow_pad_single=flow_pad_single,
    flow_num=flow_num,
    spat_size=spat_size,
    loss_wm=dict(
        type='WMLoss',
    ...
    head=dict(
        type="SparseDriveHead_flow_attn",
...
# ================== data ========================
dataset_type = "NuScenes3DDatasetEgo"
...
test_pipeline = [
    ...
    dict(
        ...
        meta_keys=["T_global", "T_global_inv", "timestamp",'ego_pose'],
    ),
]
eval_pipeline = [
    dict(
    ...
    dict(
        ...
        meta_keys=['token', 'timestamp','ego_pose']
    ),
]
...
eval_config = dict(
    **data_basic_config,
    ann_file=anno_root + 'nuscenes_infos_val_ego.pkl',
    pipeline=eval_pipeline,
    test_mode=True,
)
data_aug_conf = {
    "resize_lim": (0.40*ratio, 0.47*ratio),
...
data = dict(
    ...
    workers_per_gpu=workers_per_gpu,  # batch_size,
    train=dict(
        **data_basic_config,
        ann_file=anno_root + "nuscenes_infos_train_ego.pkl",
...
evaluation = dict(
    interval=num_iters_per_epoch*checkpoint_epoch_interval*num_epochs,
    eval_mode=eval_mode,
)
# ================== pretrained model ========================
...
load_from = "./SparseDrive/work_dirs/sparsedrive_small_stage2_flow_attn_wm_mlfuse_ego_pe_spatial-ago/iter_30730.pth"
load_from="./SparseDrive/work_dirs/sparsedrive_small_stage2_flow_attn_wm_mlfuse_ego_pe_spatial-ago/iter_6585.pth"
revise_keys = None
