...
@PIPELINES.register_module()
class LoadMultiViewImageFromMultiSweepsEgoTraj(object):
    def __init__(self,
                 sweeps_num=5,
                 color_type='color',
                 test_mode=False):
    ...
    def load_offline(self, results):
        ...
        if len(results['sweeps']['prev']) == 0:
            ego_pose_cp = copy.deepcopy(results['ego_pose'][0])
            trans_rot_cp = copy.deepcopy(results['trans_rot'][0])
            for _ in range(self.sweeps_num):
                for j in range(len(cam_types)):
                    results['img'].append(results['img'][j])
                    results['img_timestamp'].append(results['img_timestamp'][j])
                    ...
                    if 'ego2lidar' in results:
                        results['ego2lidar'].append(results['ego2lidar'][0])
                results['ego_pose'].append(copy.deepcopy(ego_pose_cp))
                results['trans_rot'].append(copy.deepcopy(trans_rot_cp))
            choices = None
        else:
            if self.test_mode:
                interval = self.test_interval
            ...
            for idx in sorted(list(choices)):
                sweep_idx = min(idx, len(results['sweeps']['prev']) - 1)
                sweep = results['sweeps']['prev'][sweep_idx]
                ego_pose_expand = results['ego_pose_expand']['prev'][sweep_idx//6]
                trans_rot_expand = results['trans_rot_expand']['prev'][sweep_idx//6]

                if len(sweep.keys()) < len(cam_types):
                    sweep = results['sweeps']['prev'][sweep_idx - 1]
                    ego_pose_expand = results['ego_pose_expand']['prev'][(sweep_idx-1)//6]
                    trans_rot_expand = results['trans_rot_expand']['prev'][(sweep_idx-1)//6]
                ...
                for sensor in cam_types:
                    ...
                    if 'ego2lidar' in results:
                        results['ego2lidar'].append(results['ego2lidar'][0])
                results['ego_pose'].append(ego_pose_expand)
                results['trans_rot'].append(trans_rot_expand)

        prev_pos = None
        prev_angle = None
        sdc_traj = []
        for idx, i in enumerate(range(len(results['ego_pose'])-1,-1,-1)):
            sdc_traj.append(copy.deepcopy(results['ego_pose'][i][:2]))
            if idx == 0:
                prev_pos = copy.deepcopy(results['ego_pose'][i][:3])
                prev_angle = copy.deepcopy(results['ego_pose'][i][-1])
                results['ego_pose'][i][:3] = 0
                results['ego_pose'][i][-1] = 0
            else:
                tmp_pos = copy.deepcopy(results['ego_pose'][i][:3])
                tmp_angle = copy.deepcopy(results['ego_pose'][i][-1])
                results['ego_pose'][i][:3] -= prev_pos
                results['ego_pose'][i][-1] -= prev_angle
                prev_pos = copy.deepcopy(tmp_pos)
                prev_angle = copy.deepcopy(tmp_angle)
        trans, rot = results['trans_rot'][-1]
        sdc_traj = convert_global_coords_to_local(
            coordinates=sdc_traj,
            translation=trans,
            rotation=rot,
        )
        results['sdc_traj'] = sdc_traj
        return results
