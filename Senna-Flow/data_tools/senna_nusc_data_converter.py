...
def _fill_trainval_infos(nusc,
    ...
    pretrained = "./llava-v1.6-vicuna-7b"
    ...
    with open('./afs_mount/idt_pc_data/guomingzhe/info/turn_tokens.json', 'r') as file:
        turn_tokens = json.load(file) # 3467
    turn_tokens = turn_tokens[800:1000]

    # for sample in tqdm(nusc.sample):
    cnt = -1
    for idx, sample in tqdm(zip(range(len(nusc.sample)), nusc.sample)):
        if sample['scene_token'] not in train_scenes:
            continue
        else:
            # cnt += 1
            # print(cnt)
            # if cnt < 1500 or cnt > 2000:
            #     continue
            if sample['token'] not in turn_tokens:
                continue
        # raise EOFError
