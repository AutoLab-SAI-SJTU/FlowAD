...
eval_data_path = './Senna/infos/senna_nusc_val.json'
base_model_path = "./Senna/ckpts"
ckpt_path = "./Senna/output-re"
merge_save_path = ckpt_path + '-merge'
# save_path = "./Senna/result/eval_result.json"
# save_path = './Senna/result/pred_result.json'
save_dir = "./result"

# load vlm model and generate
model_name = get_model_name_from_path(merge_save_path)
load_pretrained_model(ckpt_path, base_model_path, model_name='llava_lora', device_map=0, merge_save_path=merge_save_path)

tokenizer, model, image_processor, context_len = load_senna_pretrained_model(
    merge_save_path, None, model_name='llava', device_map=0, base_model_path=base_model_path)


non_lora_trainables = torch.load(os.path.join(ckpt_path, 'non_lora_trainables.bin'), map_location='cpu')
