...
from loaders.builder import build_dataloader, build_dataloader_iter
...
def main():
    ...
    if local_rank == 0:
        ...
        if os.path.isfile(os.path.join(work_dir, 'latest.pth')):
            cfgs.resume_from = os.path.join(work_dir, 'latest.pth')
        else:
            # run_name = ''
            # if not cfgs.debug:
    ...
    train_loader = build_dataloader_iter(
        train_dataset,
    ...
    val_loader = build_dataloader(
        val_dataset,
        samples_per_gpu=1,  # cfgs.data.samples_per_gpu,
        workers_per_gpu=cfgs.data.workers_per_gpu,
        num_gpus=world_size,
        dist=world_size > 1,
        shuffle=False
