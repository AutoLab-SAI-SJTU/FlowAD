...
model = dict(
    ...
    pts_bbox_head=dict(
        ...
        bbox_coder=dict(
            ...
            max_num=400,  # 300  53-detval-0.467  56-dettest
            voxel_size=voxel_size,
            score_threshold=0.0,  # 0.05
            num_classes=10),
    ), 
)
...
train_pipeline = [
    ...
    dict(type='Collect3D', keys=['gt_bboxes_3d', 'gt_labels_3d', 'img'], meta_keys=(
        'filename', 'ori_shape', 'img_shape', 'pad_shape', 'lidar2img', 'img_timestamp', 'sample_idx', 'scene_token'))
]
    ...
    dict(
        ...
        transforms=[
            ...
            dict(type='Collect3D', keys=['img'], meta_keys=(
                ...
                'lidar2img', 'img_timestamp', 'scene_token'))
        ])
]
