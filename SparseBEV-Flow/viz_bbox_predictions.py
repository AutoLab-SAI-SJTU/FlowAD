...
from models.utils import VERSION, DUMP
...
def main():
    ...
    parser.add_argument('--score_threshold', default=0.)  # 0.3
    args = parser.parse_args()

    if not os.path.exists(os.path.join(DUMP.out_dir, 'outputs')):
        os.makedirs(os.path.join(DUMP.out_dir, 'outputs'))
    ...
    for i, data in enumerate(val_loader):
        if data['img_metas'][0]._data[0][0]['sample_idx'] != '30e55a3ec6184d8cb1944b39ba19d622':
            continue
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(DUMP.out_dir, 'outputs/box_%04d.jpg' % i), dpi=200)
        plt.close()
