...
def compose_lidar2img(ego2global_translation_curr,
    ...
    if return_intrinsics:
        return lidar2img, viewpad.astype(np.float32), lidar2cam_rt.astype(np.float32)
    return lidar2img
...
@PIPELINES.register_module()
class LoadMultiViewImageFromMultiSweepsMono3D(object):
    def __init__(self,
                 sweeps_num=5,
                 color_type='color',
                 test_mode=False):
    ...
    def load_offline(self, results):
        ...
        if len(results['sweeps']['prev']) == 0:
            ...
            for idx in sorted(list(choices)):
                ...
                for j,sensor in enumerate(cam_types):
                    ...
                    lidar2img_tmp, cam_intrinsic_tmp, lidar2cam_rt = compose_lidar2img(
                    ...
                    results['lidar2img'].append(lidar2img_tmp)
                    results['intrinsics'].append(np.copy(cam_intrinsic_tmp))
                    results['extrinsics'].append(np.copy(lidar2cam_rt))
    ...
    def __call__(self, results):
        # print(self.sweeps_num, len(results['sweeps']['prev']))
