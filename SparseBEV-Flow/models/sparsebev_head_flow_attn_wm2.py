...
class SparseBEVHead_flow_attn_wm2(DETRHead):
    def __init__(self,
                 *args,
                 num_classes,
                 in_channels,
        ...
        super(SparseBEVHead_flow_attn_wm2, self).__init__(num_classes, in_channels, train_cfg=train_cfg, test_cfg=test_cfg, **kwargs)
        ...
        self.action_crossattn = nn.Transformer(d_model=in_channels, nhead=4,
                                            #    dropout=0., 
        ...
        for idx, mlvl_feat in enumerate(mlvl_feats):
            ...
            if self.flow_wm_ids:
                ...
                if idx in self.flow_wm_ids:
                    ...
                    # output_wm, mlvl_feat_flow_unfold_fut = self.world_models[layer_idx](rearrange(mlvl_feat_flow_unfold.mean(dim=2), '(b t n) c y p -> (b n) t (p y) c', b=B, t=T, n=6).flip(dims=[1]), action_query)
                    output_wm, mlvl_feat_flow_unfold_fut = self.world_models[layer_idx](rearrange(mlvl_feat_flow_unfold.mean(dim=2).mean(dim=-1), '(b t n) c y -> (b n) t y c', b=B, t=T, n=6).flip(dims=[1]),
                                                                                         rearrange(action_query, '(b t n) y c -> (b n) t y c', b=B, t=T, n=6).flip(dims=[1]))
                    ...
                    mlvl_feat_flow_unfold_fuse = self.flow_wm_mlps[layer_idx](rearrange(mlvl_feat_flow_unfold, 'b c h y p -> b h y p c')+rearrange(mlvl_feat_flow_unfold_fut.flip(dims=[1]), '(b n) t y c -> (b t n) y c', b=B, n=6, y=W//flow_patch).unsqueeze(2).unsqueeze(1))
