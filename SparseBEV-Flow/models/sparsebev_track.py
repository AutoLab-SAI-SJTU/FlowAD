...
from models.trackers_qd3dt import MOTModel


@DETECTORS.register_module()
class SparseBEV_Track(MVXTwoStageDetector):
    def __init__(self,
                 data_aug=None,
                 ...
                 test_cfg=None,
                 pretrained=None):
        super(SparseBEV_Track, self).__init__(pts_voxel_layer, pts_voxel_encoder,
        ...
        mot_cfg_path = os.path.join(current_path.rsplit('/', 2)[0], 'configs/track/mot.yaml')
        self.tracker_config = yaml.load(open(mot_cfg_path, 'r'), Loader=yaml.Loader)
        self.tracker = MOTModel(self.tracker_config)
    ...
    def forward_test(self, img_metas, img=None, **kwargs):
        ...
        if img_metas[0][0]['scene_token'] != self.prev_frame_info['scene_token']:
            ...
            # self.prev_frame_info['prev_img_feat'] = None
            # rank = kwargs['rank'] if 'rank' in kwargs.keys() else 0
            # if hasattr(self, 'tracker' + str(rank)):
            #     print('\n new scene! reset tracker')
            #     getattr(self, 'tracker' + str(rank)).reset()
            self.tracker.reset()
    ...
    def simple_test_pts(self, x, img_metas, rescale=False):
        ...
        bbox_results = [
            bbox3d2result_track(bboxes, scores, labels, sample_feats)
            for bboxes, scores, labels, sample_feats in bbox_list
        ]
        bbox_results = [self.tracker.frame_mot(result_dict, timestamp=img_metas[0]['img_timestamp'])
                            for result_dict in bbox_results]
    ...
    def simple_test_online(self, img_metas, img=None, rescale=False):
        ...
        # print([one.shape for one in img_feats])
        # box_xs = torch.tensor([10, 800])/1600
        # box_ys = torch.tensor([300, 600])/900
        # for idx in range(len(img_feats)):
        #     feat_h, feat_w = img_feats[idx].shape[-2:]
        #     box_xs_tmp = (box_xs*feat_w).long()
        #     box_ys_tmp = (box_ys*feat_h).long()
        #     img_feats[idx][:,:,:,box_ys_tmp[0]:box_ys_tmp[1], box_xs_tmp[0]:box_xs_tmp[1]] = 0
...
def bbox3d2result_track(bboxes, scores, labels, query_feats, scale_probs=None, attrs=None):
    ...
    # print(scores.shape, labels.shape, track_scores.shape)
    return result_dict
