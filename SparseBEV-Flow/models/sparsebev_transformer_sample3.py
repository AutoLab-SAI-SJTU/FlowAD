...
from .sparsebev_sampling import sampling_4d_flow2, make_sample_points
from .checkpoint import checkpoint as cp
from .csrc.wrapper import MSMV_CUDA


@TRANSFORMER.register_module()
class SparseBEVTransformer_sample3(BaseModule):
    def __init__(self, embed_dims, num_frames=8, num_points=4, num_layers=6, num_levels=4, num_classes=10, code_size=10, pc_range=[], init_cfg=None):
        ...
        super(SparseBEVTransformer_sample3, self).__init__(init_cfg=init_cfg)
    ...
    def forward(self, query_bbox, query_feat, mlvl_feats, patch_query, attn_mask, img_metas, **args):
        cls_scores, bbox_preds = self.decoder(query_bbox, query_feat, mlvl_feats, patch_query, attn_mask, img_metas)
...
class SparseBEVTransformerDecoder(BaseModule):
    ...
    def forward(self, query_bbox, query_feat, mlvl_feats, patch_query, attn_mask, img_metas):
        ...
        # query_bbox_clone = query_bbox.clone()
        for i in range(self.num_layers):
            DUMP.stage_count = i

            query_feat, cls_score, bbox_pred = self.decoder_layer(
                query_bbox, query_feat, mlvl_feats, patch_query, attn_mask, img_metas
            )
            query_bbox = bbox_pred.clone().detach()

            cls_scores.append(cls_score)
            bbox_preds.append(bbox_pred)
            # bbox_preds.append(query_bbox_clone)
...
class SparseBEVTransformerDecoderLayer(BaseModule):
    def __init__(self, embed_dims, num_frames=8, num_points=4, num_levels=4, num_classes=10, code_size=10, num_cls_fcs=2, num_reg_fcs=2, pc_range=[], init_cfg=None, num_groups=4):
        ...
        self.sampling = SparseBEVSampling(embed_dims, num_frames=num_frames, num_groups=num_groups, num_points=num_points, num_levels=num_levels, pc_range=pc_range)
        ...
        self.mixing_flow = AdaptiveMixing(in_dim=embed_dims, in_points=1 * num_frames, n_groups=num_groups, out_points=128)
        self.ffn_flow = FFN(embed_dims, feedforward_channels=512, ffn_drop=0.1)

        self.norm1 = nn.LayerNorm(embed_dims)
        self.norm2 = nn.LayerNorm(embed_dims)
        self.norm3 = nn.LayerNorm(embed_dims)

        # self.mlp_temporal_patch = nn.Linear(8, 1)
        # self.mlp_fuse_patch = nn.Linear(embed_dims*2, embed_dims)

        self.norm2_flow = nn.LayerNorm(embed_dims)
        self.norm3_flow = nn.LayerNorm(embed_dims)
    ...
    def forward(self, query_bbox, query_feat, mlvl_feats, patch_query, attn_mask, img_metas):
        ...
        sampled_feat, patch_query_NY = self.sampling(query_bbox, query_feat, mlvl_feats, patch_query, img_metas)
        query_feat = self.norm2(self.mixing(sampled_feat, query_feat))
        query_feat = self.norm3(self.ffn(query_feat))

        # print(query_feat.shape, patch_query_NY.shape, sampled_feat.shape)  # torch.Size([1, 1210, 256]) torch.Size([1, 8, 1210, 256]) torch.Size([1, 1210, 4, 32, 64])
        # patch_query_NYT = self.mlp_temporal_patch(rearrange(patch_query_NY, 'b t q c -> b q c t')).squeeze(-1)
        # query_feat = self.mlp_fuse_patch(torch.cat([query_feat, patch_query_NYT], dim=-1))

        query_feat = self.norm2_flow(self.mixing_flow(rearrange(patch_query_NY, 'b t q (g c) -> b q g t c', g=self.num_groups), query_feat))
        query_feat = self.norm3_flow(self.ffn_flow(query_feat))
...
class SparseBEVSampling(BaseModule):
    ...
    def inner_forward(self, query_bbox, query_feat, mlvl_feats, patch_query, img_metas):
        '''
        query_bbox: [B, Q, 10]
        query_feat: [B, Q, C]
        '''
        ...
        # sampling
        sampled_feats, patch_query_NY = sampling_4d_flow2(
            sampling_points,
            mlvl_feats,
            patch_query,
            scale_weights,
        ...
        return sampled_feats, patch_query_NY

    def forward(self, query_bbox, query_feat, mlvl_feats, patch_query, img_metas):
        if self.training and query_feat.requires_grad:
            return cp(self.inner_forward, query_bbox, query_feat, mlvl_feats, patch_query, img_metas, use_reentrant=False)
        else:
            return self.inner_forward(query_bbox, query_feat, mlvl_feats, patch_query, img_metas)
