...
class BiMultiHeadAttention(nn.Module):
    ...
    def __init__(self,
        ...
        if self.only_visual:
            for one in self.values_l_proj.parameters():
                one.requires_grad = False
            for one in self.out_l_proj.parameters():
                one.requires_grad = False
            for one in self.values_v_proj.parameters():
                one.requires_grad = False

    def _shape(self, tensor: Tensor, seq_len: int, bsz: int):
        return tensor.view(bsz, seq_len, self.num_heads,
                           self.head_dim).transpose(1, 2).contiguous()
    def _shape1(self, tensor: Tensor, seq_len: int, bsz: int ,num_cam: int=6):
        return tensor.view(bsz, num_cam, seq_len, self.num_heads,
                           self.head_dim).transpose(2, 3).contiguous()
    ...
    def forward(
        ...
        num_cam, tgt_len, _ = vision.size()
        query_states = self.v_proj(vision) * self.scale
        key_states = self._shape(self.l_proj(lang), -1, num_cam)#6,26,256  6,26,1024  6,26,4,256,  6,4,26,256
        value_v_states = self._shape(self.values_v_proj(vision), -1, num_cam)
        value_l_states = self._shape(self.values_l_proj(lang), -1, num_cam)
        proj_shape = (self.num_heads, -1, self.head_dim)
        proj_shape1 = (self.num_heads * num_cam, -1, self.head_dim)
        query_states = self._shape(query_states, tgt_len,num_cam).view(*proj_shape1)#1,6,30825,1024   24,30825,256
        key_states = key_states.view(*proj_shape1)
        value_v_states = value_v_states.view(*proj_shape1)
        value_l_states = value_l_states.view(*proj_shape1)

        src_len = key_states.size(1)
        attn_weights = torch.bmm(query_states, key_states.transpose(1, 2))

        if attn_weights.size() != (self.num_heads * num_cam, tgt_len, src_len):
            raise ValueError(
                f'Attention weights should be of '
                f'size {(self.num_heads, tgt_len, src_len)}, '
        ...
        if attention_mask_l is not None:
            ...
            attention_mask = attention_mask.expand(num_cam, 1, tgt_len, src_len)
            attention_mask = attention_mask.masked_fill(
                attention_mask == 0, -9e15)

            if attention_mask.size() != (num_cam, 1, tgt_len, src_len):
                raise ValueError('Attention mask should be of '
                                 f'size {(num_cam, 1, tgt_len, src_len)}')
            attn_weights = attn_weights.view(num_cam, self.num_heads, tgt_len,
                                             src_len) + attention_mask
            attn_weights = attn_weights.view(self.num_heads * num_cam, tgt_len,
                                             src_len)
        ...
        if attn_output_v.size() != (self.num_heads * num_cam, tgt_len,
                                    self.head_dim):
            raise ValueError(
                '`attn_output_v` should be of '
                f'size {(self.num_heads, tgt_len, self.head_dim)}, '
                f'but is {attn_output_v.size()}')

        if attn_output_l.size() != (self.num_heads * num_cam, src_len,
                                    self.head_dim):
            raise ValueError(
                '`attn_output_l` should be of size '
                f'{(self.num_heads, src_len, self.head_dim)}, '
                f'but is {attn_output_l.size()}')

        attn_output_v = attn_output_v.view(num_cam, self.num_heads, tgt_len,
                                           self.head_dim)
        attn_output_v = attn_output_v.transpose(2, 3)
        attn_output_v = attn_output_v.reshape(num_cam, tgt_len, self.embed_dim)

        attn_output_l = attn_output_l.view(num_cam, self.num_heads, src_len,
                                           self.head_dim)
        attn_output_l = attn_output_l.transpose(2, 3)
        attn_output_l = attn_output_l.reshape(num_cam, src_len, self.embed_dim)
...
class BiAttentionBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.gamma_l = nn.Parameter(
            init_values * torch.ones(l_dim), requires_grad=True if not self.only_visual else False)
