...
class SparseBEVTransformer_Track(BaseModule):
    def __init__(self, embed_dims, num_frames=8, num_points=4, num_layers=6, num_levels=4, num_classes=10, code_size=10, pc_range=[], init_cfg=None):
        ...
        super(SparseBEVTransformer_Track, self).__init__(init_cfg=init_cfg)
    ...
    def forward(self, query_bbox, query_feat, mlvl_feats, attn_mask, img_metas, **args):
        cls_scores, bbox_preds, sample_feats = self.decoder(query_bbox, query_feat, mlvl_feats, attn_mask, img_metas)

        cls_scores = torch.nan_to_num(cls_scores)
        bbox_preds = torch.nan_to_num(bbox_preds)
        sample_feats = torch.nan_to_num(sample_feats)

        return cls_scores, bbox_preds, sample_feats
...
class SparseBEVTransformerDecoder(BaseModule):
    ...
    def forward(self, query_bbox, query_feat, mlvl_feats, attn_mask, img_metas):
        cls_scores, bbox_preds, sample_feats = [], [], []
        ...
        for i in range(self.num_layers):
            ...
            sample_feats.append(query_feat)
