...
from .world_model_hd3 import RSSM
...
class FlowCrossAttention(nn.Module):
    def __init__(self, dim, heads = 4, dim_head = 64, dropout = 0., with_res=False):
        super().__init__()
        inner_dim = dim_head *  heads
        project_out = not (heads == 1 and dim_head == dim)
...
class SparseBEVHead_flow_attn_wm_noquery_sample(DETRHead):
    def __init__(self,
                 *args,
                 num_classes,
                 in_channels,
        ...
        super(SparseBEVHead_flow_attn_wm_noquery_sample, self).__init__(num_classes, in_channels, train_cfg=train_cfg, test_cfg=test_cfg, **kwargs)
        ...
        self.flow_attns = nn.ModuleList([
            FlowAttention(dim=in_channels, heads = 8, dim_head = in_channels//8, dropout = 0.) for flow_attn_id in self.flow_attn_ids
        ])
        ...
        self.flow_wm_attns = nn.ModuleList([
            nn.Transformer(d_model=in_channels, nhead=8,
                    #    dropout=0., 
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        for idx, mlvl_feat in enumerate(mlvl_feats):
            ...
            if self.flow_wm_ids:
                ...
                #     action_query = self.action_crossattn(rearrange(mlvl_feat_flow_unfold.mean(dim=2), 'b c y p -> (y p) b c'), action_query.transpose(0,1)).transpose(0,1)
                #     # action_query = self.action_crossattn.decoder(action_query.transpose(0,1), rearrange(mlvl_feat_flow_unfold.mean(dim=2), 'b c y p -> (y p) b c')).transpose(0,1)
                #     # action_query = self.action_crossattn(action_query.transpose(0,1), rearrange(mlvl_feat_flow_unfold.mean(dim=2), 'b c y p -> (y p) b c')).transpose(0,1)
                #     # print(action_query.shape)
                if idx in self.flow_wm_ids:
                    ...
                    # mlvl_feat_flow_unfold_fuse = self.flow_query_attns[layer_idx](mlvl_feat_flow_unfold.mean(dim=1), flow_query)
                    # mlvl_feat_flow_unfold_fuse = self.flow_wm_mlps[layer_idx](rearrange(mlvl_feat_flow_unfold, 'b c h y p -> b h y p c')+rearrange(mlvl_feat_flow_unfold_fut.flip(dims=[1]), '(b n) t (p y) c -> (b t n) y p c', b=B, n=6, p=flow_patch, y=W//flow_patch).unsqueeze(1))
                    mlvl_feat_flow_unfold_fuse = torch.cat([rearrange(mlvl_feat_flow_unfold, 'b c h y p -> p (b h y) c'), rearrange(mlvl_feat_flow_unfold_fut.flip(dims=[1]), '(b n) t (h y p) c -> p (b t n h y) c', b=B, n=6, y=W//flow_patch, p=flow_patch)], dim=-1)
                    ...
                    mlvl_feat_ori_unfold = F.unfold(mlvl_feat.flatten(0,2), kernel_size=(H, flow_patch), stride=flow_patch)
                    mlvl_feat_ori_unfold = mlvl_feat_ori_unfold.reshape(B*TN, C, H, flow_patch, W//flow_patch).transpose(-1,-2)
                    mlvl_feat_flow_unfold = torch.cat([mlvl_feat_flow_unfold, mlvl_feat_ori_unfold], dim=1)
