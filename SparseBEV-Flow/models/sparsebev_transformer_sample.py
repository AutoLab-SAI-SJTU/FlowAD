...
from .sparsebev_sampling import sampling_4d_flow, make_sample_points
from .checkpoint import checkpoint as cp
from .csrc.wrapper import MSMV_CUDA


@TRANSFORMER.register_module()
class SparseBEVTransformer_sample(BaseModule):
    def __init__(self, embed_dims, num_frames=8, num_points=4, num_layers=6, num_levels=4, num_classes=10, code_size=10, pc_range=[], init_cfg=None):
        ...
        super(SparseBEVTransformer_sample, self).__init__(init_cfg=init_cfg)
    ...
    def forward(self, query_bbox, query_feat, mlvl_feats, attn_mask, img_metas, **args):
...
class SparseBEVTransformerDecoder(BaseModule):
    ...
    def forward(self, query_bbox, query_feat, mlvl_feats_all, attn_mask, img_metas):
        ...
        mlvl_feats = []
        mlvl_feats_flow = []
        for lvl, feat in enumerate(mlvl_feats_all):
            B, TN, LGC, H, W = feat.shape  # [B, TN, GC, H, W]
            L = 2
            N, T, G, C = 6, TN // 6, 4, LGC // (L*4)
            feat = feat.reshape(B, T, N, L, G, C, H, W)

            if MSMV_CUDA:  # Our CUDA operator requires channel_last
                feat = feat.permute(3, 0, 1, 4, 2, 6, 7, 5)  # [B, T, G, N, H, W, C]
                feat = feat.reshape(L, B*T*G, N, H, W, C)
            else:  # Torch's grid_sample requires channel_first
                feat = feat.permute(3, 0, 1, 4, 5, 2, 6, 7)  # [B, T, G, C, N, H, W]
                feat = feat.reshape(L, B*T*G, C, N, H, W)

            # mlvl_feats[lvl] = feat.contiguous()
            mlvl_feats.append(feat[0].contiguous())
            mlvl_feats_flow.append(feat[1].contiguous())
        ...
        for i in range(self.num_layers):
            ...
            query_feat, cls_score, bbox_pred = self.decoder_layer(
                query_bbox, query_feat, mlvl_feats, mlvl_feats_flow, attn_mask, img_metas
            )
...
class SparseBEVTransformerDecoderLayer(BaseModule):
    def __init__(self, embed_dims, num_frames=8, num_points=4, num_levels=4, num_classes=10, code_size=10, num_cls_fcs=2, num_reg_fcs=2, pc_range=[], init_cfg=None):
        ...
        self.mixing_flow = AdaptiveMixing(in_dim=embed_dims, in_points=num_points * num_frames, n_groups=4, out_points=128)
    ...
    def forward(self, query_bbox, query_feat, mlvl_feats, mlvl_feats_flow, attn_mask, img_metas):
        """
        ...
        sampled_feat, sampled_feat_flow = self.sampling(query_bbox, query_feat, mlvl_feats, mlvl_feats_flow, img_metas)
        query_feat = self.norm2(self.mixing(sampled_feat, query_feat))
        query_feat = self.norm3(self.ffn(query_feat))

        query_feat = self.norm2_flow(self.mixing_flow(sampled_feat_flow, query_feat))
...
class SparseBEVSampling(BaseModule):
    ...
    def inner_forward(self, query_bbox, query_feat, mlvl_feats, mlvl_feats_flow, img_metas):
        '''
        query_bbox: [B, Q, 10]
        query_feat: [B, Q, C]
        '''
        ...
        # sampling
        sampled_feats, sampled_feats_flow = sampling_4d_flow(
            sampling_points,
            mlvl_feats,
            mlvl_feats_flow,
            scale_weights,
        ...
        return sampled_feats, sampled_feats_flow

    def forward(self, query_bbox, query_feat, mlvl_feats, mlvl_feats_flow, img_metas):
        if self.training and query_feat.requires_grad:
            return cp(self.inner_forward, query_bbox, query_feat, mlvl_feats, mlvl_feats_flow, img_metas, use_reentrant=False)
        else:
            return self.inner_forward(query_bbox, query_feat, mlvl_feats, mlvl_feats_flow, img_metas)
