...
from .csrc.wrapper import msmv_sampling, msmv_sampling_pytorch, msmv_sampling_pytorch_flow, msmv_sampling_pytorch_flow2
...
def make_sample_points_cycer(query_bbox, pc_range):
    '''
    query_bbox: [B, Q, 10]
    offset: [B, Q, num_points, 4], normalized by stride
    '''
    ...
    sample_xyz = xyz[:, :, None, :]  # [B, Q, P, 3]

    return sample_xyz  # [B, Q, P, 3]


def sampling_4d(sample_points, mlvl_feats, scale_weights, lidar2img, image_h, image_w, eps=1e-5, msmv_cuda=True):
    """
    Args:
    ...
    # print('sampling', sample_points_cam.shape, torch.min(sample_points_cam), torch.max(sample_points_cam))
    ...
    final = msmv_sampling(mlvl_feats, sample_points_cam, scale_weights, msmv_cuda=msmv_cuda)
...
def sampling_4d_flow(sample_points, mlvl_feats, mlvl_feats_flow, scale_weights, lidar2img, image_h, image_w, eps=1e-5, msmv_cuda=True):
    """
    Args:
        sample_points: 3D sampling points in shape [B, Q, T, G, P, 3]
        mlvl_feats: list of multi-scale features from neck, each in shape [B*T*G, C, N, H, W]
    ...
    # multi-scale multi-view grid sample
    # final = msmv_sampling(mlvl_feats, sample_points_cam, scale_weights, msmv_cuda=msmv_cuda)
    final, final_flow = msmv_sampling_pytorch_flow(mlvl_feats, mlvl_feats_flow, sample_points_cam, scale_weights)

    # reorganize the sampled features
    # C = final.shape[2]  # [BTG, Q, C, P]
    final = rearrange(final, '(b t g) q c p -> b q g (t p) c', b=B, t=T, g=G)
    # final = final.reshape(B, T, G, Q, C, P)
    # final = final.permute(0, 3, 2, 1, 5, 4)  # B Q G T P C
    # final = final.flatten(3, 4)  # [B, Q, G, FP, C]
    final_flow = rearrange(final_flow, '(b t g) q c p -> b q g (t p) c', b=B, t=T, g=G)

    return final, final_flow

def sampling_4d_flow2(sample_points, mlvl_feats, patch_query, scale_weights, lidar2img, image_h, image_w, eps=1e-5, msmv_cuda=True):
    """
    Args:
        sample_points: 3D sampling points in shape [B, Q, T, G, P, 3]
        mlvl_feats: list of multi-scale features from neck, each in shape [B*T*G, C, N, H, W]
    ...
    B, T, N, Y, C = patch_query.shape
    i_view_patch = torch.argmax(valid_mask.mean(dim=3), dim=-1)  # [B, T, Q]
    patch_query_N = torch.gather(patch_query, 2, i_view_patch[..., None, None].expand(B, T, Q, Y, C))  # B T Q Y C
    i_patch = (torch.clamp(sample_points_cam.mean(dim=3)[...,0,0], min=0., max=1.)*(Y-1)).long()
    patch_query_NY = torch.gather(patch_query_N, 3, i_patch[..., None, None].expand(B, T, Q, 1, C)).squeeze(dim=3)
    ...
    # multi-scale multi-view grid sample
    final = msmv_sampling(mlvl_feats, sample_points_cam, scale_weights, msmv_cuda=msmv_cuda)
    # final, final_flow = msmv_sampling_pytorch_flow2(mlvl_feats, patch_query, sample_points_cam, scale_weights)
    ...
    return final, patch_query_NY

def sampling_4d_cycer(sample_points, lidar2img, image_h, image_w, eps=1e-5, msmv_cuda=True):
    """
    Args:
        sample_points: 3D sampling points in shape [B, Q, T, G, P, 3]
        mlvl_feats: list of multi-scale features from neck, each in shape [B*T*G, C, N, H, W]
    ...
    return torch.cat([sample_points_cam, homo_nonzero], dim=-1)[:,:,:,:,0], valid_mask[:,:,:,:,0]
...
def sampling_4d_cycer_mean(sample_points, lidar2img, image_h, image_w, eps=1e-5, msmv_cuda=True):
    """
    Args:
        sample_points: 3D sampling points in shape [B, Q, T, G, P, 3]
        mlvl_feats: list of multi-scale features from neck, each in shape [B*T*G, C, N, H, W]
    ...
    return torch.cat([sample_points_cam, homo_nonzero], dim=-1).mean(dim=4), valid_mask.mean(dim=4)
