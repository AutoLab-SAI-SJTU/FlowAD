...
class SparseBEVHead_flow_attn_wm_mlfuse_patch_predict(DETRHead):
    def __init__(self,
                 *args,
                 num_classes,
                 in_channels,
        ...
        super(SparseBEVHead_flow_attn_wm_mlfuse_patch_predict, self).__init__(num_classes, in_channels, train_cfg=train_cfg, test_cfg=test_cfg, **kwargs)
        ...
        self.patch_selfattn = FlowAttention(dim=in_channels, heads = 22, dim_head = in_channels, dropout = 0., with_res=False, channel_fuse=True)
        self.patch_crossattn = FlowCrossAttention(dim=in_channels, heads = 22, dim_head = in_channels, dropout = 0., with_res=False, channel_fuse=True)

        self.patch_query = nn.Parameter(torch.zeros(6, 22, self.in_channels))
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        # print(patch_query.shape)

        for idx, (mlvl_feat_flow_unfold, (H, W, flow_patch)) in enumerate(zip(mlvl_feats_flow_unfold, H_W_Ps)):
            if idx in self.flow_wm_ids:
                # print(flow_query.shape, mlvl_feat_flow_unfold.shape)
                layer_idx = self.flow_wm_ids.index(idx)

                # output_wm, mlvl_feat_flow_unfold_fut = self.world_models[layer_idx](rearrange(mlvl_feat_flow_unfold, '(b t n) c h y p -> (b n) t (h p y) c', b=B, t=T, n=6).flip(dims=[1]))
                # output_wm, mlvl_feat_flow_unfold_fut = self.world_models[layer_idx](rearrange(mlvl_feat_flow_unfold.mean(dim=2).mean(dim=-1), '(b t n) c y -> (b n) t y c', b=B, t=T, n=6).flip(dims=[1]))
                ...
                # print(mlvl_feat_flow_unfold_fut.shape)

                outputs_wm.append(output_wm)

                # mlvl_feat_flow_unfold_fuse = self.flow_wm_attns[layer_idx](rearrange(mlvl_feat_flow_unfold_fut.flip(dims=[1]), '(b n) t (h y p) c -> p (b t n h y) c', b=B, n=6, y=W//flow_patch, p=flow_patch), rearrange(mlvl_feat_flow_unfold, 'b c h y p -> p (b h y) c'))
                # mlvl_feat_flow_unfold = rearrange(mlvl_feat_flow_unfold_fuse, 'p (b h y) c -> b c h y p', b=B*TN, h=H, y=W//flow_patch)  # [BTN, C, H, patch*3, W/patch]
        ...
        query_bbox_init = self.init_query_bbox.weight.clone()  # [Q, 10]
        #query_bbox[..., :3] = query_bbox[..., :3].sigmoid()

        # query denoising
        B = mlvl_feats[0].shape[0]
        query_bbox, query_feat, attn_mask, mask_dict = self.prepare_for_dn_input(B, query_bbox_init, self.label_enc, img_metas)

        ## patch classification
        # organize projections matrix and copy to CUDA
        lidar2imgs = np.asarray([m['lidar2img'] for m in img_metas]).astype(np.float32)
        lidar2imgs = torch.from_numpy(lidar2imgs).to(query_bbox.device)  # [B, N, 4, 4]
        image_h, image_w, _ = img_metas[0]['img_shape'][0]
        outs['lidar2img'] = lidar2imgs
        outs['img_hw'] = (image_h, image_w)


        # B, TN, _, _ = lidar2imgs.shape
        # N = 6
        # T = TN//N
        # eps = 1e-5
        # lidar2imgs = lidar2imgs.reshape(B, T, N, 4, 4)[:, 0]  # [B,N,4,4]
        # for bs_id, (lidar2img, q_bboxes) in enumerate(zip(lidar2imgs, query_bbox.clone())):
        #     ones = torch.ones_like(q_bboxes[..., :1])
        #     num_gt = q_bboxes.size(0)
        #     q_bboxes_align = torch.cat([q_bboxes[..., :3], ones], dim=-1)[None,:,:].expand(N,num_gt,4).unsqueeze(-1)  # [N, num_gt, 4, 1]
        #     lidar2img_align = lidar2img.unsqueeze(1).repeat(1,num_gt,1,1)
        #     print(lidar2img_align.shape, q_bboxes_align.shape)
        #     q_bboxes_cam = torch.matmul(lidar2img_align, q_bboxes_align).squeeze(-1)  # N, num_gt, 4

        #     # homo coord -> pixel coord
        #     homo = q_bboxes_cam[..., 2:3]
        #     homo_nonzero = torch.maximum(homo, torch.zeros_like(homo) + eps)
        #     q_bboxes_cam = q_bboxes_cam[..., 0:2] / homo_nonzero  # [B, T, N, Q, GP, 2]

        #     # normalize
        #     q_bboxes_cam[..., 0] /= image_w
        #     q_bboxes_cam[..., 1] /= image_h

        #     # check if out of image
        #     valid_mask = ((homo > eps) \
        #         & (q_bboxes_cam[..., 1:2] > 0.0)
        #         & (q_bboxes_cam[..., 1:2] < 1.0)
        #         & (q_bboxes_cam[..., 0:1] > 0.0)
        #         & (q_bboxes_cam[..., 0:1] < 1.0)
        #     ).squeeze(-1).float()  # [B, T, N, Q, GP]
        #     print(valid_mask, valid_mask.shape)
        #     print(valid_mask.sum(dim=1))
        #     # print([torch.where(one>0)[0] for one in valid_mask])
        #     indices_cam = [torch.where(one>0)[0] for one in valid_mask]
    ...
    def _get_target_single(self,
        ...
        # DETR
        # print(bbox_pred.shape, pos_inds)
    ...
    def loss(self,
        ...
        lidar2imgs = preds_dicts['lidar2img']
        image_h, image_w = preds_dicts['img_hw']
        B, TN, _, _ = lidar2imgs.shape
        N = 6
        T = TN//N
        eps = 1e-5
        ...
        for bs_id, (lidar2img, gt_bboxes) in enumerate(zip(lidar2imgs, gt_bboxes_list)):
            ones = torch.ones_like(gt_bboxes[..., :1])
            num_gt = gt_bboxes.size(0)
            gt_bboxes_align = torch.cat([gt_bboxes[..., :3], ones], dim=-1)[None,:,:].expand(N,num_gt,4).unsqueeze(-1)  # [N, num_gt, 4, 1]
            lidar2img_align = lidar2img.unsqueeze(1).repeat(1,num_gt,1,1)
            
            gt_bboxes_cam = torch.matmul(lidar2img_align, gt_bboxes_align).squeeze(-1)  # N, num_gt, 4

            # homo coord -> pixel coord
            homo = gt_bboxes_cam[..., 2:3]
            homo_nonzero = torch.maximum(homo, torch.zeros_like(homo) + eps)
            gt_bboxes_cam = gt_bboxes_cam[..., 0:2] / homo_nonzero  # [B, T, N, Q, GP, 2]

            # normalize
            gt_bboxes_cam[..., 0] /= image_w
            gt_bboxes_cam[..., 1] /= image_h

            # check if out of image
            valid_mask = ((homo > eps) \
                & (gt_bboxes_cam[..., 1:2] > 0.0)
                & (gt_bboxes_cam[..., 1:2] < 1.0)
                & (gt_bboxes_cam[..., 0:1] > 0.0)
                & (gt_bboxes_cam[..., 0:1] < 1.0)
            ).squeeze(-1).float()  # [N, Q]
            # print(valid_mask.sum(dim=0))
            indices_cam = [torch.where(one>0)[0] for one in valid_mask]
        # raise EOFError
        ...
        all_gt_bboxes_list = [[gt_bboxes_list[0][indice_cam]] for indice_cam in indices_cam]
        all_gt_labels_list = [[gt_labels_list[0][indice_cam]] for indice_cam in indices_cam]
        all_gt_bboxes_ignore_list = [gt_bboxes_ignore for _ in indices_cam]
        ...
        loss_dict['loss_wm'] = sum(losses_wm)
        # print(loss_dict)
        # raise EOFError
        return loss_dict
