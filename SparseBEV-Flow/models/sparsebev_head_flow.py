...
class SparseBEVHead_flow(DETRHead):
    def __init__(self,
                 *args,
                 num_classes,
                 in_channels,
        ...
        super(SparseBEVHead_flow, self).__init__(num_classes, in_channels, train_cfg=train_cfg, test_cfg=test_cfg, **kwargs)
        ...
        self.flow_mlps = nn.ModuleList([
            nn.Linear(3*self.flow_patches[flow_id], self.flow_patches[flow_id]) for flow_id in self.flow_ids
        ])
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        for idx, mlvl_feat in enumerate(mlvl_feats):
            ...
            feat_pad_left = mlvl_feat_reshaped[:,:,self.ids_pad_left_right[:,0],:,:,-flow_patch:]
            feat_pad_right = mlvl_feat_reshaped[:,:,self.ids_pad_left_right[:,1],:,:,:flow_patch]
            mlvl_feat_pad_reshaped = torch.cat([feat_pad_left, mlvl_feat_reshaped, feat_pad_right], dim=-1)

            mlvl_feat_pad_unfold = F.unfold(mlvl_feat_pad_reshaped.flatten(0,2), kernel_size=(H, 3*flow_patch), stride=flow_patch)
            # print(mlvl_feat_pad_reshaped.shape, mlvl_feat_pad_unfold.shape)
            mlvl_feat_flow_unfold = self.flow_mlps[self.flow_ids.index(idx)](mlvl_feat_pad_unfold.reshape(B*TN, C, H, 3*flow_patch, W//flow_patch).transpose(3,4))
            ...
            # print(mlvl_feat_flow_fold.shape)
