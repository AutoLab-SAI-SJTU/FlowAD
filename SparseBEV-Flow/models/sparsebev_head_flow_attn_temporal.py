...
class SparseBEVHead_flow_attn_temporal(DETRHead):
    def __init__(self,
                 *args,
                 num_classes,
                 in_channels,
        ...
        super(SparseBEVHead_flow_attn_temporal, self).__init__(num_classes, in_channels, train_cfg=train_cfg, test_cfg=test_cfg, **kwargs)
        ...
        self.flow_attn_tem_ids = flow_attn_tem_ids
        ...
        self.flow_tem_attns = nn.ModuleList([
            FlowAttention(dim=in_channels, heads = 4, dim_head = in_channels//4, dropout = 0.) for flow_attn_tem_id in self.flow_attn_tem_ids
        ])
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        for idx, mlvl_feat in enumerate(mlvl_feats):
            ...
            if idx in self.flow_attn_tem_ids:
                mlvl_feat_flow_unfold = rearrange(mlvl_feat_flow_unfold, '(b t n) c h y p -> (b n h y) (t p) c', b=B, t=T, n=6)
                mlvl_feat_flow_unfold = self.flow_tem_attns[self.flow_attn_tem_ids.index(idx)](mlvl_feat_flow_unfold)
                mlvl_feat_flow_unfold = rearrange(mlvl_feat_flow_unfold, '(b n h y) (t p) c -> (b t n) c h y p', b=B, n=6, t=T, h=H, y=W//flow_patch, p=flow_patch)
