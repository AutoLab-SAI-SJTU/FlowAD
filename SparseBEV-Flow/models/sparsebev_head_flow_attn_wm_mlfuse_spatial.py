...
class SparseBEVHead_flow_attn_wm_mlfuse_spatial(DETRHead):
    def __init__(self,
                 *args,
                 num_classes,
                 in_channels,
        ...
        super(SparseBEVHead_flow_attn_wm_mlfuse_spatial, self).__init__(num_classes, in_channels, train_cfg=train_cfg, test_cfg=test_cfg, **kwargs)
        ...
        self.flow_wm_convs = nn.ModuleList([
            nn.Conv2d(in_channels*3, in_channels, kernel_size=3, padding=1) for flow_wm_id in self.flow_wm_ids
        ])
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        patch_query_spatial = patch_query.clone()
        ...
        for idx, (mlvl_feat_flow_unfold, (H, W, flow_patch)) in enumerate(zip(mlvl_feats_flow_unfold, H_W_Ps)):
            if idx in self.flow_wm_ids:
                ...
                output_wm_spatial, mlvl_feat_flow_unfold_fut_spatial = self.world_models_spatial[layer_idx](rearrange(mlvl_feat_flow_unfold.mean(dim=2).mean(dim=-1), '(b t n) c y -> (b n) y t c', b=B, t=T, n=6), rearrange(patch_query_spatial, '(b t n) y c -> (b n) y t c', b=B, t=T, n=6))
                ...
                mlvl_feat_flow_unfold_fuse = torch.cat([rearrange(mlvl_feat_flow_unfold, 'b c h y p -> b h y p c'),
                                                        rearrange(mlvl_feat_flow_unfold_fut.flip(dims=[1]), '(b n) t y c -> (b t n) y c', b=B, n=6, y=W//flow_patch).unsqueeze(2).unsqueeze(1).repeat(1,H,1,flow_patch,1),
                                                        rearrange(mlvl_feat_flow_unfold_fut_spatial, '(b n) y t c -> (b t n) y c', b=B, n=6, y=W//flow_patch).unsqueeze(2).unsqueeze(1).repeat(1,H,1,flow_patch,1)], dim=-1)
