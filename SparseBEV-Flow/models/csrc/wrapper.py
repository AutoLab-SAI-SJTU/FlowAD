...
def msmv_sampling_pytorch(mlvl_feats, sampling_locations, scale_weights):
    ...
    """
    # print(scale_weights.shape, len(mlvl_feats))
...
def msmv_sampling_pytorch_flow(mlvl_feats, mlvl_feats_flow, sampling_locations, scale_weights):
    """
    value: [B, N, H1W1 + H2W2..., C]
    sampling_locations: [B, Q, P, 3]
    scale_weights: [B, Q, P, 4]
    ...
    final = torch.zeros([B, C, Q, P], device=mlvl_feats[0].device)
    final_flow = torch.zeros([B, C, Q, P], device=mlvl_feats[0].device)

    for lvl, (feat, feat_flow) in enumerate(zip(mlvl_feats, mlvl_feats_flow)):
        out = F.grid_sample(
            feat, sampling_locations, mode='bilinear',
            padding_mode='zeros', align_corners=True,
        )[..., 0]  # [B, C, Q, P]
        ...
        out_flow = F.grid_sample(
            feat_flow, sampling_locations, mode='bilinear',
            padding_mode='zeros', align_corners=True,
        )[..., 0]  # [B, C, Q, P]
        out_flow = out_flow * scale_weights[..., lvl].reshape(B, 1, Q, P)
        final_flow += out_flow

    return final.permute(0, 2, 1, 3), final_flow.permute(0, 2, 1, 3)

def msmv_sampling_pytorch_flow2(mlvl_feats, patch_query, sampling_locations, scale_weights):
    """
    value: [B, N, H1W1 + H2W2..., C]
    sampling_locations: [B, Q, P, 3]
    scale_weights: [B, Q, P, 4]
    ...
    print([one.shape for one in mlvl_feats], sampling_locations.shape)
    raise EOFError
...
def msmv_sampling(mlvl_feats, sampling_locations, scale_weights, msmv_cuda=True):
    if len(mlvl_feats) == 4 and MSMV_CUDA and msmv_cuda:
        return MSMVSamplingC2345.apply(*mlvl_feats, sampling_locations, scale_weights)
    elif len(mlvl_feats) == 5 and MSMV_CUDA and msmv_cuda:
