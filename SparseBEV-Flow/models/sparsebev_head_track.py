...
class SparseBEVHead_Track(DETRHead):
    def __init__(self,
                 *args,
                 num_classes,
                 in_channels,
        ...
        super(SparseBEVHead_Track, self).__init__(num_classes, in_channels, train_cfg=train_cfg, test_cfg=test_cfg, **kwargs)
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        cls_scores, bbox_preds, sample_feats = self.transformer(
            query_bbox,
            query_feat,
            mlvl_feats,
            attn_mask=attn_mask,
        ...
        if mask_dict is not None and mask_dict['pad_size'] > 0:  # if using query denoising
            ...
            sample_feats = sample_feats[-1, :, :mask_dict['pad_size'], :]
            outs = {
    ...
    def get_bboxes(self, preds_dicts, img_metas, rescale=False):
        ...
        for i in range(num_samples):
            ...
            scores = preds['scores']
            labels = preds['labels']
            sample_feats = preds['sample_feats']
            ret_list.append([bboxes, scores, labels, sample_feats])
        return ret_list
