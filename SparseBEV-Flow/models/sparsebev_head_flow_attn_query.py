...
class SparseBEVHead_flow_attn_query(DETRHead):
    def __init__(self,
                 *args,
                 num_classes,
                 in_channels,
        ...
        super(SparseBEVHead_flow_attn_query, self).__init__(num_classes, in_channels, train_cfg=train_cfg, test_cfg=test_cfg, **kwargs)
        ...
        self.flow_query_attn_ids = flow_query_attn_ids
        self.flow_pad_single = flow_pad_single
        self.flow_pad_sum = 1 + 2 * flow_pad_single
        self.flow_h_pixel = flow_h_pixel
        ...
        if self.flow_query_attn_ids:
            self.flow_query = nn.Embedding(22 * flow_h_pixel, self.in_channels)  # 176/8 88/4 44/2 22/1
            self.flow_query_attn_finegrain = FlowCrossAttention(dim=in_channels, heads = 4, dim_head = in_channels//4, dropout = 0.)
            self.flow_query_attns = nn.ModuleList([
                FlowCrossAttention(dim=in_channels, heads = 4, dim_head = in_channels//4, dropout = 0.) for flow_query_attn_id in self.flow_query_attn_ids
            ])
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        for idx, mlvl_feat in enumerate(mlvl_feats):
            ...
            if idx in self.flow_attn_ids:
                ...
                mlvl_feat_flow_unfold = rearrange(mlvl_feat_flow_unfold, '(b h y) p c -> b c h p y', b=B*TN, h=H, y=W//flow_patch)  # [BTN, C, H, patch*3, W/patch]

            mlvl_feat_flow_unfold = self.flow_mlps[self.flow_ids.index(idx)](mlvl_feat_flow_unfold.transpose(3,4))  # [BTN, C, H, W/patch, patch]

            if self.flow_query_attn_ids:
                if idx == 0:
                    flow_query = self.flow_query.weight.reshape(-1, self.flow_h_pixel, self.in_channels).unsqueeze(0).repeat(B*TN, 1, 1, 1).flatten(0,1)  # [BTN, 22, 8, 256] -> [BTN*22, 8, 256]
                    flow_query = self.flow_query_attn_finegrain(flow_query, rearrange(mlvl_feat_flow_unfold.mean(dim=-1), 'b c h y -> (b y) h c'))
                if idx in self.flow_query_attn_ids:
                    mlvl_feat_flow_unfold = rearrange(mlvl_feat_flow_unfold, 'b c h y p -> (b y) (h p) c')
                    
                    mlvl_feat_flow_unfold = self.flow_query_attns[self.flow_query_attn_ids.index(idx)](mlvl_feat_flow_unfold, flow_query)
                    mlvl_feat_flow_unfold = rearrange(mlvl_feat_flow_unfold, '(b y) (h p) c -> b c h y p', b=B*TN, h=H, y=W//flow_patch)  # [BTN, C, H, patch*3, W/patch]
