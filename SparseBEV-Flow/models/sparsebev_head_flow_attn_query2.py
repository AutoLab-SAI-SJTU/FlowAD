...
class SparseBEVHead_flow_attn_query2(DETRHead):
    def __init__(self,
                 *args,
                 num_classes,
                 in_channels,
        ...
        super(SparseBEVHead_flow_attn_query2, self).__init__(num_classes, in_channels, train_cfg=train_cfg, test_cfg=test_cfg, **kwargs)
        ...
        if self.flow_query_attn_ids:
            self.flow_query = nn.Parameter(torch.zeros(6, 22, self.in_channels))
            trunc_normal_(self.flow_query, std=.02)
            # self.flow_query = nn.Embedding(6 * 22, self.in_channels)  # 176/8 88/4 44/2 22/1
            self.flow_query_attn_finegrain = FlowCrossAttention(dim=in_channels, heads = 4, dim_head = in_channels//4, dropout = 0., with_res=True)
            self.flow_query_attns = nn.ModuleList([
                FlowCrossAttention(dim=in_channels, heads = 4, dim_head = in_channels//4, dropout = 0., with_res=True) for flow_query_attn_id in self.flow_query_attn_ids
            ])
            self.flow_query_mlps = nn.ModuleList([
                nn.Sequential(
                    FeedForward(dim=in_channels, hidden_dim=in_channels, dropout=0.),
                    nn.LayerNorm(in_channels)
                ) for flow_query_attn_id in self.flow_query_attn_ids
            ])
    ...
    def forward(self, mlvl_feats, img_metas):
        ...
        for idx, mlvl_feat in enumerate(mlvl_feats):
            ...
            if idx in self.flow_attn_ids:
                layer_idx = self.flow_attn_ids.index(idx)
                mlvl_feat_flow_unfold = rearrange(mlvl_feat_flow_unfold, 'b c h p y -> (b h y) p c')
                mlvl_feat_flow_unfold = self.flow_attns[layer_idx](mlvl_feat_flow_unfold)
            ...
            if self.flow_query_attn_ids:
                if idx == 0:
                    flow_query = self.flow_query.unsqueeze(0).repeat(B*T, 1, 1, 1).flatten(0,1)  # [N, 22, 256] -> [BTN, 22, 256]
                    # flow_query = self.flow_query.weight.reshape(1, 6, W//flow_patch, self.embed_dims).repeat(B*T, 1, 1, 1).flatten(0,1)  # [N, 22, 256] -> [BTN, 22, 256]
                    # print(flow_query.shape, mlvl_feat_flow_unfold.shape, rearrange(mlvl_feat_flow_unfold.mean(dim=-1), 'b c h y -> (b y) h c').shape)
                    flow_query = self.flow_query_attn_finegrain(flow_query, rearrange(mlvl_feat_flow_unfold.mean(dim=2), 'b c y p -> b (y p) c'))
                if idx in self.flow_query_attn_ids:
                    layer_idx = self.flow_query_attn_ids.index(idx)
                    mlvl_feat_flow_unfold = rearrange(mlvl_feat_flow_unfold, 'b c h y p -> b h (y p) c')
                    mlvl_feat_flow_unfold_fuse = self.flow_query_attns[layer_idx](mlvl_feat_flow_unfold.mean(dim=1), flow_query)
                    mlvl_feat_flow_unfold_fuse = self.flow_query_mlps[layer_idx](mlvl_feat_flow_unfold_fuse.unsqueeze(1)+mlvl_feat_flow_unfold)
