...
class MOTModel(torch.nn.Module):
    def __init__(self, configs):
        ...
        self.memo_tracklet_frames = 10  # 10
        ...
        self.track_bbox_iou = 'box3d'  # 'box3d / bev / giou / iou3d'
    ...
    def frame_mot(self, origin_data, timestamp=None):
        ...
        boxes_vxys = torch.cat([bbox_vx, bbox_vy], dim=-1)
        ...
        if boxes_3d.size(0) > 0 and not self.empty:
            ...
            memo_trackers, memo_query_feats, memo_bev_feats, memo_img_feats, memo_ids, memo_vs, memo_vxys = self.memo
                ...
                if self.track_bbox_iou == 'bev':
                    ...
                    if self.with_vxy:
                        depth_weight = F.pairwise_distance(
                            torch.cat([boxes_3d, boxes_vxys], dim=-1)[:, None, :],
                            torch.cat([memo_boxes_3d_predict, memo_vxys], dim=-1)[None, ...])
                    else:
                        depth_weight = F.pairwise_distance(
                            boxes_3d[:, None, :],
                            memo_boxes_3d_predict[None, ...])
                    scores_iou = torch.exp(-depth_weight / 10.0)
                elif self.track_bbox_iou == 'giou':
                    dets_xy_box = get_xy_box(boxes_3d)
                    memo_dets_xy_box = get_xy_box(memo_boxes_3d_predict)
                    ...
                    scores_iou = all_iou(dets_xy_box_expand, memo_dets_xy_box_expand, xyxy=True, GIoU=True).reshape(
                        len_dets_xy_box, len_memo_dets_xy_box)
                    ...
                    len_dets_xy_box = dets_xy_box.size(0)
                    len_memo_dets_xy_box = memo_dets_xy_box.size(0)
                    dets_xy_box_expand = dets_xy_box.unsqueeze(1).repeat(1, len_memo_dets_xy_box, 1).flatten(0,
                                                                                                             1).unsqueeze(
                        0)
                    memo_dets_xy_box_expand = memo_dets_xy_box.unsqueeze(0).repeat(len_dets_xy_box, 1, 1).flatten(0,
                                                                                                                  1).unsqueeze(
                        0)
                    scores_iou = diff_iou_rotated_3d(dets_xy_box_expand, memo_dets_xy_box_expand).reshape(
                        len_dets_xy_box, len_memo_dets_xy_box)
                else:
        ...
        self.update_memo(ids, boxes_3d, depth_uncertainty, query_feats, bev_feats, img_feats,
                         labels_3d, boxes_vxys, cur_frame=self.frame_count)
        self.frame_count += 1
    ...
    def update_memo(self, ids, boxes_3d, depth_uncertainty, query_feats, bev_feats, img_feats, labels_3d, boxes_vxys, cur_frame):
        tracklet_inds = ids > -1

        # update memo
        for tid, box_3d, d_uncertainty, query_feat, bev_feat, img_feat, label_3d, box_vxys in zip(
                ...
                img_feats[tracklet_inds], labels_3d[tracklet_inds], boxes_vxys[tracklet_inds]):
            tid = int(tid)
            if tid in self.tracklets.keys():
                ...
                self.tracklets[tid]['query_feat'] += self.memo_momentum * (
                            query_feat - self.tracklets[tid]['query_feat'])
                ...
                self.tracklets[tid]['velocity_xy'] = box_vxys
        ...
        self.backdrops.insert(
            ...
            dict(
                ...
                velocity_xy=boxes_vxys[backdrop_inds],
            ))
    ...
    def memo(self):
        ...
        for k, v in self.tracklets.items():
            ...
            memo_vxys.append(v['velocity_xy'][None, :])
        ...
        for backdrop in self.backdrops:
            ...
            memo_vxys.append(backdrop['velocity_xy'])
        ...
        memo_vxys = torch.cat(memo_vxys, dim=0)
