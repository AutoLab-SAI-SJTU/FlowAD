...
import numpy as np
import torch
from torch import mm, zeros, eye, linalg
# import numpy.linalg as linalg
try:
    from filterpy.stats import logpdf
    from filterpy.common import pretty_str
except:
    print('environment no filterpy')
...
class KalmanFilter(object):
    ...
    **Example**
    ...
    def __init__(self, dim_x, dim_z, dim_u=0, device='cpu'):
        if dim_x < 1:
        ...
        self.x = zeros((dim_x, 1), device=self.device, dtype=torch.float32)        # state
        self.P = eye(dim_x, device=self.device, dtype=torch.float32)               # uncertainty covariance
        self.Q = eye(dim_x, device=self.device, dtype=torch.float32)               # process uncertainty
        self.B = None                     # control transition matrix
        self.F = eye(dim_x, device=self.device, dtype=torch.float32)               # state transition matrix
        self.H = zeros((dim_z, dim_x), device=self.device, dtype=torch.float32)    # Measurement function
        self.R = eye(dim_z, device=self.device, dtype=torch.float32)               # state uncertainty
        self._alpha_sq = 1.               # fading memory control
        self.M = torch.zeros((dim_z, dim_z), device=device, dtype=torch.float32) # process-measurement cross correlation
        self.z = np.array([[None]*self.dim_z]).T

        # gain and residual are computed during the innovation step. We
        # save them so that in case you want to inspect them for various
        # purposes
        self.K = torch.zeros((dim_x, dim_z), device=device) # kalman gain
        self.y = torch.zeros((dim_z, 1), device=device)
        self.S = torch.zeros((dim_z, dim_z), device=device) # system uncertainty
        self.SI = torch.zeros((dim_z, dim_z), device=device) # inverse system uncertainty

        # identity matrix. Do not alter this.
        self._I = torch.eye(dim_x, device=device)
    ...
    def predict(self, u=None, B=None, F=None, Q=None):
        ...
        if B is not None and u is not None:
            self.x = mm(F, self.x) + mm(B, u)
        else:
            self.x = mm(F, self.x)

        # P = FPF' + Q
        self.P = self._alpha_sq * mm(mm(F, self.P), F.T) + Q
    ...
    def update(self, z, R=None, H=None):
        ...
        self.K = mm(PHT, self.SI)

        # x = x + Ky
        # predict new x with residual scaled by the kalman gain
        self.x = self.x + mm(self.K, self.y)
        ...
        I_KH = self._I - mm(self.K, H)
        self.P = mm(mm(I_KH, self.P), I_KH.T) + mm(mm(self.K, R), self.K.T)
    ...
    def predict_steadystate(self, u=0, B=None):
        ...
        # x = Fx + Bu
        if B is not None:
            self.x = mm(self.F, self.x) + mm(B, u)
        else:
            self.x = mm(self.F, self.x)
    ...
    def update_correlated(self, z, R=None, H=None):
        ...
        if self.x.ndim == 1 and z.shape == (1, 1):
            z = z[0]

        if z.shape == (): # is it scalar, e.g. z=3 or z=np.array(3)
            z = torch.from_numpy(np.asarray([z])).to(self.device)
        ...
        # project system uncertainty into measurement space
        self.S = mm(H, PHT) + mm(H, self.M) + mm(self.M.T, H.T) + R
        self.SI = self.inv(self.S)

        # K = PH'inv(S)
        # map system uncertainty into kalman gain
        self.K = mm(PHT + self.M, self.SI)

        # x = x + Ky
        # predict new x with residual scaled by the kalman gain
        self.x = self.x + mm(self.K, self.y)
        self.P = self.P - mm(self.K, mm(H, self.P) + self.M.T)
    ...
    def rts_smoother(self, Xs, Ps, Fs=None, Qs=None, inv=linalg.inv):
        ...
        x, P, Pp = Xs.clone(), Ps.clone(), Ps.clone()
        for k in range(n-2, -1, -1):
            Pp[k] = mm(mm(Fs[k+1], P[k]), Fs[k+1].T) + Qs[k+1]

            #pylint: disable=bad-whitespace
            K[k]  = mm(mm(P[k], Fs[k+1].T), inv(Pp[k]))
            x[k] += mm(K[k], x[k+1] - mm(Fs[k+1], x[k]))
            P[k] += mm(mm(K[k], P[k+1] - Pp[k]), K[k].T)
    ...
    def get_prediction(self, u=0):
        ...
        if isinstance(u, int):
            ...
            item = mm(self.B, u)
        x = mm(self.F, self.x) + item
        P = self._alpha_sq * mm(mm(self.F, self.P), self.F.T) + self.Q
        return (x, P)
    ...
    def get_update(self, z=None):
       ...
       """
        ...
        # P = (I-KH)P(I-KH)' + KRK'
        I_KH = self._I - mm(K, H)
        P = mm(mm(I_KH, P), I_KH.T) + mm(mm(K, R), K.T)
    ...
    def residual_of(self, z):
        ...
        """
        return z - mm(self.H, self.x_prior)
    ...
    def mahalanobis(self):
        ...
        if self._mahalanobis is None:
            self._mahalanobis = sqrt(float(mm(mm(self.y.T, self.SI), self.y)))
    ...
    def log_likelihood_of(self, z):
        ...
        return logpdf(z, mm(self.H, self.x), self.S)
    ...
    def test_matrix_dimensions(self, z=None, H=None, R=None, F=None, Q=None):
        ...
        if z_shape == (): # scalar or np.array(scalar)
            assert Hx.ndim == 1 or Hx.shape == (1, 1), \
            "shape of z should be {}, not {} for the given H".format(
                Hx.shape, z_shape)

        elif Hx.shape == (1,):
            assert z_shape[0] == 1, 'Shape of z must be {} for the given H'.format(Hx.shape)

        else:
            assert (z_shape == Hx.shape or
                    (len(z_shape) == 1 and Hx.shape == (z_shape[0], 1))), \
                    "shape of z should be {}, not {} for the given H".format(
                        Hx.shape, z_shape)

        if Hx.ndim > 1 and Hx.shape != (1, 1):
            assert Hx.shape == z_shape, \
...
def update(x, P, z, R, H=None, return_all=False):
    ...
    if H is None:
        H = torch.from_numpy(np.array([1])).to(x.device)

    if np.isscalar(H):
        H = torch.from_numpy(np.array(H)).to(x.device)

    Hx = torch.atleast_1d(mm(H, x))
    ...
    try:
        I_KH = torch.eye(KH.shape[0]) - KH
    except:
        I_KH = torch.from_numpy(np.array([1 - KH])).to(x.device)
    P = mm(mm(I_KH, P), I_KH.T) + mm(mm(K, R), K.T)


    if return_all:
        # compute log likelihood
        log_likelihood = logpdf(z, mm(H, x), S)
...
def predict(x, P, F=1, Q=0, u=0, B=1, alpha=1.):
    ...
    if np.isscalar(F):
        F = torch.from_numpy(np.array(F)).to(x.device)
    x = mm(F, x) + mm(B, u)
    P = (alpha * alpha) * mm(mm(F, P), F.T) + Q
...
def rts_smoother(Xs, Ps, Fs, Qs):
    ...
    for k in range(n-2, -1, -1):
        pP[k] = mm(mm(Fs[k], P[k]), Fs[k].T) + Qs[k]

        #pylint: disable=bad-whitespace
        K[k]  = mm(mm(P[k], Fs[k].T), linalg.inv(pP[k]))
        x[k] += mm(K[k], x[k+1] - mm(Fs[k], x[k]))
        P[k] += mm(mm(K[k], P[k+1] - pP[k]), K[k].T)
