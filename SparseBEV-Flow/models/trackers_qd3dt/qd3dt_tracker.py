...
class MOTModel(torch.nn.Module):
    def __init__(self, configs):
        ...
        if self.tracker_model_name == 'LSTM3DTracker':
            ...
            # self.lstm_ckpt_name = os.path.join(current_path.rsplit('/', 5)[0], 'ckpts/batch128_min10_seq10_dim7_VeloLSTM_nuscenes_100_linear.pth')  # 'J:/BDD100K/batch128_min10_seq10_dim7_VeloLSTM_nuscenes_100_linear.pth'
    ...
    # def frame_mot_single(self, origin_data, timestamp=None):
    ...
    #     bbox_x, bbox_y, bbox_z, bbox_w, bbox_l, bbox_h, bbox_o = boxes_3d[:, 0:1], boxes_3d[:, 1:2], boxes_3d[:, 2:3], boxes_3d[:, 3:4], boxes_3d[:, 4:5], boxes_3d[:, 5:6], boxes_3d[:, 6:7]
    ...
    #                  self.feat_affinity_weight * (scores_query_feat * self.weight_query_feat + scores_bev_feat * self.weight_bev_feat + scores_img_feat * self.weight_img_feat)
    ...
    #             for i in range(boxes_3d.size(0)):
    ...
    #                         if depth_uncertainty[i] > self.obj_score_thr:
    #                             ids[i] = tid
    #                             scores[:i, memo_ind] = 0
    ...
    # def frame_mot_multi(self, origin_data, timestamp=None):
    def frame_mot(self, origin_data, timestamp=None):
        """ For each frame input, generate the latest mot results
        Args:
            input_data (FrameData): input data, including detection bboxes and ego information
        ...
        if boxes_3d.size(0) > 0 and not self.empty:
            ...
            # print([torch.sum(scores_single>self.match_score_thr) for scores_single in scores_list])
