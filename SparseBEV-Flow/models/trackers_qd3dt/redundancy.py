...
import numpy as np, models.trackers_qd3dt.utils as utils
...
class RedundancyModule:
    ...
    def motion_model_redundancy(self, trk: Tracklet, input_data: FrameData, time_lag):
        ...
        # new
        len_pred = 1
        len_cand = len(candidate_dets)
        if len_pred == 0 or len_cand == 0:
            dists = np.zeros((len_pred, len_cand))[0]
        else:
            det_embeds = []
            for d, det in enumerate(candidate_dets):
                det_embeds.append(det.query_feat)
            track_embeds = []
            track_embeds.append(trk.query_feat)
            dim = det.query_feat.size(-1)
            det_embeds = torch.stack(det_embeds, dim=0)
            track_embeds = torch.stack(track_embeds, dim=0)
            # print('\n1: ', det_embeds.shape, track_embeds.shape)

            det_embeds = det_embeds.unsqueeze(1).repeat(1, len_pred, 1).reshape(-1, dim)
            track_embeds = track_embeds.unsqueeze(0).repeat(len_cand, 1, 1).reshape(-1, dim)
            # print('\n2: ', det_embeds.shape, track_embeds.shape)
            embeds_matrix = F.cosine_similarity(det_embeds, track_embeds).reshape(len_pred, len_cand)

            dists = 1 - F.sigmoid(embeds_matrix).cpu()

        if (len(dists) == 0 or torch.min(dists) > 0.3):
    ...
    def motion_model_redundancy_ago(self, trk: Tracklet, input_data: FrameData, time_lag):
        # get the motion model prediction / current state
        pred_bbox = trk.get_state()
        ...
        if len_pred == 0 or len_cand == 0:
            ...
            pred_bboxes = []
            pred_bboxes.append(BBox.bbox2array_lidar_center(pred_bbox))
            candidate_bboxes = []
            for d, dtrket in enumerate(candidate_dets):
                candidate_bboxes.append(BBox.bbox2array_lidar_center(dtrket))

            pred_bboxes = torch.stack(pred_bboxes, dim=0).unsqueeze(1).repeat(1, len_cand, 1).reshape(1, -1, 7)
            candidate_bboxes = torch.stack(candidate_bboxes, dim=0).unsqueeze(0).repeat(len_pred, 1, 1).reshape(1, -1, 7)
            dists = diff_iou_rotated_3d(pred_bboxes, candidate_bboxes).reshape(len_pred, len_cand)[0].cpu().numpy()
    ...
    def bipartite_infer(self, input_data: FrameData, tracklets):
        ...
        matched, unmatched_dets, unmatched_trks = associate_dets_to_tracks(dets, trk_preds,
                                                                           'bipartite', 'giou', 1 - self.det_threshold,
                                                                           None)
