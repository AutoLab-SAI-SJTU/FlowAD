...
def buffer_bbox_func(bboxes, scale_ids, buffer_sizes=[0.5, 0.4, 0.3, 0.2, 0.1]):
    buffered_bboxes = bboxes.clone()
    buffer_sizes = torch.tensor(buffer_sizes, device=bboxes.device)

    buffer_sizes = buffer_sizes[scale_ids]
...
class MOTModel(torch.nn.Module):
    def __init__(self, configs):
        ...
        if self.tracker_model_name == 'LSTM3DTracker':
            ...
            self.lstm_ckpt_name = 'checkpoints/batch128_min10_seq10_dim7_VeloLSTM_nuscenes_100_linear.pth'
        else:
        ...
        self.memo_tracklet_frames = 3  # 10
        ...
        self.with_bev_feat = False
        self.with_img_feat = False
        ...
        self.weight_query_feat = 1 / 3  # 1 / 3
        self.weight_bev_feat = 1/3  # 1 / 3
        self.weight_img_feat = 1/3  # 1 / 3
        self.match_algo = 'greedy'  # 'greedy / hungarian'

        # self.with_vxy = False
        self.buffer_bbox = False
        self.with_scales = False

        self.bev_w = 150
        self.bev_l = 150
        self.bev_h = 4
        self.bev_coordinate = True
    ...
    def frame_mot(self, origin_data, timestamp=None):
        ...
        img_feats = raw_data['query_feats']  # img_feats
        bev_feats = raw_data['query_feats']  # bev_feats
        ...
        if boxes_3d.size(0) > 0 and not self.empty:
            ...
            memo_labels_3d, memo_boxes_3d, \
            memo_trackers, memo_query_feats, memo_bev_feats, memo_img_feats, memo_ids, memo_vs = self.memo

            # print('begin', boxes_3d.shape, query_feats.shape, memo_boxes_3d.shape, memo_query_feats.shape)
            ...
            if self.with_bbox_iou:
                ...
                if self.track_bbox_iou == 'bev':
                    if self.buffer_bbox:
                        dets_xy_box = get_xy_box(buffer_bbox_func(boxes_3d, scale_ids))
                        memo_dets_xy_box = get_xy_box(buffer_bbox_func(memo_boxes_3d_predict, memo_scale_ids))
                    else:
                    ...
                    # print(self.plat, boxes_3d.shape, memo_boxes_3d_predict.shape, scores_iou.shape)
                    ...
                    scores_iou = all_iou(dets_xy_box_expand, memo_dets_xy_box_expand, xyxy=True, CIoU=True).reshape(len_dets_xy_box, len_memo_dets_xy_box)
                    # scores_iou = F.sigmoid(scores_iou)
            ...
            # print(scores_query_feat.shape, scores_bev_feat.shape, scores_img_feat.shape, scores_iou.shape, scores_depth.shape)
            scores_list = [
                scores_query_feat * self.weight_query_feat + scores_bev_feat * self.weight_bev_feat + scores_img_feat * self.weight_img_feat,
                scores_iou * scores_depth,
            ]
            # scores_list = [scores_query_feat, scores_bev_feat, scores_img_feat, scores_iou * scores_depth]
            filters = (scores_iou > 0.0).float() * (scores_depth > 0.0).float() * scores_cats * scores_scales
            scores_list = torch.stack([scores * filters for scores in scores_list], dim=0)

            num_det = boxes_3d.size(0)
            recoder = torch.ones(num_det)
            ...
            #     if self.match_algo == 'greedy':
            #         for i in range(len(remain_idx)):
            #             conf, memo_ind = torch.max(scores[map_dict[i], :], dim=0)
            #             tid = memo_ids[memo_ind]
            #             # Matching confidence
            #             if conf > self.match_score_thr:
            #                 # Update existing tracklet
            ...
            #             i = matched_indices[0][idx]
            #             memo_ind = matched_indices[1][idx]
            #             conf = scores[map_dict[i], memo_ind]
            #             tid = memo_ids[memo_ind]
            #             if conf > self.match_score_thr and tid > -1:
            #                 recoder[map_dict[i]] = 0
            #                 # Keep object with high 3D objectness
            #                 if depth_uncertainty[map_dict[i]] > self.obj_score_thr:
            #                     ids[map_dict[i]] = tid
            #                     scores[:, map_dict[i], memo_ind] = 0
            #                     scores[:, map_dict[i] + 1:, memo_ind] = 0
            #                 else:
            #                     # Reduce FP w/ low objectness but high match conf
            #                     if conf > self.nms_conf_thr:
            #                         ids[map_dict[i]] = -2
        ...
        self.update_memo(ids, boxes_3d, depth_uncertainty, query_feats, bev_feats, img_feats,
                         labels_3d, cur_frame=self.frame_count)
        self.frame_count += 1
        ...
        # tracklet_inds = update_ids > -1
        # result_dict = dict(
        #     boxes_3d=update_boxes_3d[tracklet_inds].to('cpu'),
        #     scores_3d=update_scores_3d[tracklet_inds].cpu(),
        #     labels_3d=update_labels_3d[tracklet_inds].cpu(),
        #     track_scores=update_scores_3d[tracklet_inds].cpu(),
        #     track_ids=update_ids[tracklet_inds].cpu(),
        # )

        return result_dict

    def update_memo(self, ids, boxes_3d, depth_uncertainty, query_feats, bev_feats, img_feats, labels_3d, cur_frame):
        tracklet_inds = ids > -1

        # update memo
        for tid, box_3d, d_uncertainty, query_feat, bev_feat, img_feat, label_3d in zip(
                ...
                img_feats[tracklet_inds], labels_3d[tracklet_inds]):
            tid = int(tid)
