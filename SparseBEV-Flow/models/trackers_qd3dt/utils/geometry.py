...
def iou3d(box_a, box_b):
    boxa_corners = torch.stack(BBox.box2corners2d(box_a)).cpu()
    boxb_corners = torch.stack(BBox.box2corners2d(box_b))[:, :2].cpu()
    reca, recb = Polygon(boxa_corners), Polygon(boxb_corners)
...
def giou3d(box_a: BBox, box_b: BBox):
    boxa_corners = torch.stack(BBox.box2corners2d(box_a))[:, :2].cpu()
    boxb_corners = torch.stack(BBox.box2corners2d(box_b))[:, :2].cpu()
    reca, recb = Polygon(boxa_corners), Polygon(boxb_corners)
    ha, hb = box_a.h, box_b.h
    za, zb = box_a.z, box_b.z
...
def m_distance(det, trk, trk_inv_innovation_matrix=None):
    ...
    diff = (det_array - trk_array).unsqueeze(1)
    corrected_yaw_diff = diff_orientation_correction(diff[3])
    diff[3] = corrected_yaw_diff

    if trk_inv_innovation_matrix is not None:
        result = \
            torch.sqrt(torch.matmul(torch.matmul(diff.T, trk_inv_innovation_matrix), diff)[0][0])
    else:
        result = torch.sqrt(torch.dot(diff.T, diff))
    return result
